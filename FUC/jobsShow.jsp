		...
		<table class="table table-bordered table-hover" style="text-align: center; border: 1px solid #dddddd">
			<thead>
				...
				<tr>
					...
					<td colspan="3"><h5><%= jobs.getJobTitle() %></h5></td>
				</tr>
				<tr>
					<td style="background-color: #fafafa; color: #000000; width: 80px;"><h5>작성자</h5></td>
					<td colspan="3"><h5><%= jobs.getUserID() %></h5></td>
				</tr>
				<tr>
					<td style="background-color: #fafafa; color: #000000; width: 80px;"><h5>작성일</h5></td>
					<td><h5><%= jobs.getJobDate() %></h5></td>
					<td style="background-color: #fafafa; color: #000000; width: 80px;"><h5>조회수</h5></td>
					<td><h5><%= jobs.getJobHit() + 1 %></h5></td>
				</tr>
				<tr>
					<td style="vartical-align: middle; min-height: 150px; background-color: #fafafa; color: #000000; width: 80px;"><h5>내용</h5></td>
					<td colspan="3" style="text-align: left"><h5><%= jobs.getJobContent() %></h5></td>
				</tr>
				<tr>
					<td style="background-color: #fafafa; color: #000000; width: 80px;"><h5>첨부파일</h5></td>
					<td colspan="3"><h5><a href="jobsDownload.jsp?jobID=<%= jobs.getJobID() %>"><%= jobs.getJobFile() %></a></h5></td>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td colspan="5" style="text-align: right;">
						<a href="jobs.jsp" class="btn btn-primary">목록</a>
						<a href="jobsReply.jsp?jobID=<%= jobs.getJobID() %>" class="btn btn-primary">답변</a>
						<%
							if(userID != null && userID.equals(jobs.getUserID())) {
						%>
							<a href="jobsUpdate.jsp?jobID=<%= jobs.getJobID() %>" class="btn btn-primary">수정</a>
							<a href="jobsDelete?jobID=<%= jobs.getJobID() %>" class="btn btn-primary" onclick="return confirm('이 게시물을 삭제하시겠습니까?')">삭제</a>
						<%
							}
						%>
					</td>
